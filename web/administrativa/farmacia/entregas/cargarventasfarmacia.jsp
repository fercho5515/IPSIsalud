...
<%@page import="pac.conexion" %>
  ...
  request.setCharacterEncoding("UTF-8");         
                    ...
                    //       System.out.println(where);
                            ...
                            total = conexiondb.countRec("m.serial", "medicamentos m,forma_farmaceutica f,via_admin v,unidad_medida u,inventario_farmacia ifa,lab_farmaceutico lf,ventasfarmacia vfa where  ifa.cod_medicamento=m.serial and lf.idlab_farmaceutico=ifa.laboratorio and m.forma_farmaceutica=f.id_forma_farmaceutica and m.via_administracion=v.id_via_admin and m.unidad_medida=u.id_unidad_medida and vfa.id_medic_inventario=ifa.serial_inven and vfa.id_person='"+request.getParameter("cedulan")+"' and vfa.tipo_id='"+request.getParameter("tipoc")+"' "+where+"");
                 //      System.out.println("total : "+total);
                            ...
                            strQuery = "select DISTINCT m.serial,lf.idlab_farmaceutico,m.producto,m.cod_atc,v.descripcion as via_ad,f.descripcion as forma_f,concat(m.concentracion,' ',u.descripcion) as unioon,lf.nombre as laboratorio,ifa.cod_barras,ifa.lote,ifa.num_factura,vfa.cantidad as existencias,vfa.valor from medicamentos m,forma_farmaceutica f,via_admin v,unidad_medida u,inventario_farmacia ifa,lab_farmaceutico lf,ventasfarmacia vfa where  ifa.cod_medicamento=m.serial and lf.idlab_farmaceutico=ifa.laboratorio and m.forma_farmaceutica=f.id_forma_farmaceutica and m.via_administracion=v.id_via_admin and m.unidad_medida=u.id_unidad_medida and vfa.id_medic_inventario=ifa.serial_inven and vfa.id_person='"+request.getParameter("cedulan")+"' and vfa.tipo_id='"+request.getParameter("tipoc")+"' "+where+" ORDER BY "+sidx + " " +sord +" LIMIT "+start+" , "+limit;
