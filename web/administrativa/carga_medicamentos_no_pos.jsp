...
<%@page import="pac.conexion" %>
  ...
  <%
                         ...
                         //  System.out.println(where);
                            ...
                            total = conexiondb.countRec(" m.serial", " medicamentos m where (m.plan_beneficios=1 or m.plan_beneficios=2 or m.plan_beneficios=3) and m.serial not in (select medi1 from autorizacion_no_pos where id_soli_no_post='"+sln+"') "+where+"");
                            ...
                            strQuery = "select          m.serial,m.producto,m.via_adimin  as via_ad,forma_farmaceutica as forma_f,concat(m.cantidad,' ',m.unidad_medida)     as unioon from medicamentos m where (m.plan_beneficios=1 or m.plan_beneficios=2 or m.plan_beneficios=3) and m.serial not in (select medi1 from autorizacion_no_pos where id_soli_no_post='"+sln+"') "+where+" ORDER BY "+sidx + " " +sord +" LIMIT "+start+" , "+limit;
                          
                            //   System.out.println(strQuery);
                            
                            rs = conexiondb.Consulta(strQuery);

                           // total = conexiondb.countRec("DISTINCT m.serial", " medicamentos m,forma_farmaceutica f,via_admin v,unidad_medida u where m.forma_farmaceutica=f.id_forma_farmaceutica and m.via_administracion=v.id_via_admin and m.unidad_medida=u.id_unidad_medida and (m.plan_beneficios=1 or m.plan_beneficios=2 or m.plan_beneficios=3) and m.serial not in (select medi1 from autorizacion_no_pos where id_soli_no_post='"+sln+"') "+where+"");
