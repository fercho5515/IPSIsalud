...
<%@page import="pac.conexion" %>
  ...
  <%                        request.setCharacterEncoding("UTF-8");
                         ...
                         String historia="";
                         String wher="";
                          if(request.getParameter("hist")!=null ){idtipo=request.getParameter("hist");} 
                          
                               if(idtipo.compareTo("1")==0){historia=",historia_clinica h";wher=" and pe.serial=h.serial_peciente ";}
                          else if(idtipo.compareTo("2")==0){historia=",historia_clinica h";wher=" and pe.serial=h.serial_peciente ";}
                          else if(idtipo.compareTo("3")==0){historia=",historia_clinica h";wher=" and pe.serial=h.serial_peciente ";}
                          else if(idtipo.compareTo("4")==0){historia=",histo_nutricion h";wher=" and pe.serial=h.serial_paciente ";}
                          else if(idtipo.compareTo("5")==0){historia=",histo_psicologica h";wher=" and pe.serial=h.serial ";}
                          else if(idtipo.compareTo("6")==0){historia=",histo_trabajo_social h";wher=" and pe.serial=h.id_paciente ";}
                                                       else{historia=",historia_clinica h";wher=" and pe.serial=h.serial_peciente and pe.serial='-1' ";}
                            
                             total = conexiondb.countRec("pe.serial", " personas pe,pacientes pa"+historia+" where pe.serial=pa.serial_persona "+wher+" "+where+"");
                        //     System.out.println("total "+total);
                            ...
                            strQuery = "select pe.serial,id_person,p_nom,s_nom,p_ape,s_ape from personas pe,pacientes pa"+historia+" where pe.serial=pa.serial_persona "+wher+" "+where+" ORDER BY "+sidx + " " +sord +" LIMIT "+start+" , "+limit;
