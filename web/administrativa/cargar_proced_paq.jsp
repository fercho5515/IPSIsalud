...
<%@page import="pac.conexion" %>
  ...
  <%                        request.setCharacterEncoding("UTF-8");
                            ...
                            if(busca.compareTo("true")==0){
                                        ...
                                        try {
                                             ...
                                             for(int i=0;i<Integer.parseInt(""+msg.size());i++){   ///ago un ciclo para todad las restricciones
                                                ...
                                                if(campo.compareTo("pt_descripcion")==0){
                                                campo=campo.replace("_",".");
                                                }
                                                if(campo.compareTo("t_nombre")==0){
                                                campo=campo.replace("_",".");
                                                }
                         ...
                         //  System.out.println(where);
                            ...
                            total = conexiondb.countRec("pt.idprocedimientos_tari","procedimientos_tari pt, cups cu,tarifarios t,tari_proced tp,procedimientos_paquete pp where pt.cod_cups=cu.id_cups and tp.id_proced=pt.idprocedimientos_tari and t.id_tarifarios=tp.id_tarifario and pp.cod_actividad=tp.id_proced and pp.cod_tarifario=tp.id_tarifario and pp.cod_paq_int='"+(String)session.getAttribute("id_paq")+"' "+where+"");
                        //    System.out.println("total "+total);
                            ...
                            strQuery = "select pt.idprocedimientos_tari,cu.cup_verdadero,pt.codigo_maipos,pt.descripcion as d1,t.nombre as n1, pp.tiempo_mesese,pp.cantidad from procedimientos_tari pt, cups cu,tarifarios t,tari_proced tp,procedimientos_paquete pp where pt.cod_cups=cu.id_cups and tp.id_proced=pt.idprocedimientos_tari and t.id_tarifarios=tp.id_tarifario and pp.cod_actividad=tp.id_proced and pp.cod_tarifario=tp.id_tarifario and pp.cod_paq_int='"+(String)session.getAttribute("id_paq")+"' "+where+" ORDER BY "+sidx + " " +sord +" LIMIT "+start+" , "+limit;
                          //  System.out.println(strQuery);
                           ...
                           // total = conexiondb.countRec("pt.idprocedimientos_tari","procedimientos_tari pt, cups cu,tarifarios t,tari_proced tp,procedimientos_paquete pp where pt.cod_cups=cu.id_cups and tp.id_proced=pt.idprocedimientos_tari and t.id_tarifarios=tp.id_tarifario and pp.cod_actividad=tp.id_proced and pp.cod_tarifario=tp.id_tarifario and pp.cod_paq_int='"+(String)session.getAttribute("id_paq")+"' "+where+"");
                            ...
                            while(rs.next()){
                                ...
                                json = json + "\n{";
                                json = json + "\"id\":\""+rs.getInt("idprocedimientos_tari")+"\",";
                                json = json + "\"cell\":["+rs.getInt("idprocedimientos_tari")+"";
                                json = json + ",\""+rs.getString("cup_verdadero")+"\"";                                
                                json = json + ",\""+rs.getString("codigo_maipos")+"\"";
                                json = json + ",\""+rs.getString("d1")+"\"";
                                json = json + ",\""+rs.getString("n1")+"\"";
                                json = json + ",\""+rs.getString("tiempo_mesese")+"\"";
