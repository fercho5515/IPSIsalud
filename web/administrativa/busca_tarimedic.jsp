...
<%@page import="pac.conexion" %>
  ...
  request.setCharacterEncoding("UTF-8");         
                       ...
                       //    System.out.println(where);
                            ...
                            total = conexiondb.countRec("m.serial", "medicamentos m,unidad_medida um,forma_farmaceutica ff,tari_med tm,lab_farmaceutico lf  where lf.idlab_farmaceutico=tm.id_laboratorio and um.id_unidad_medida=m.unidad_medida and ff.id_forma_farmaceutica=m.forma_farmaceutica and tm.id_med=m.serial and tm.id_tari='"+(String)session.getAttribute("tarifariomed")+"' "+where+"");
                          //  out.print("====>"+total);
                            if( total>0 ) {
                            ...
                            strQuery = "select m.serial,m.cum,m.cod_atc,m.producto,m.concentracion,um.descripcion as d1,ff.descripcion as d2,m.presentacion,tm.precio,lf.nombre as lab from medicamentos m,unidad_medida um,forma_farmaceutica ff,tari_med tm,lab_farmaceutico lf  where lf.idlab_farmaceutico=tm.id_laboratorio and um.id_unidad_medida=m.unidad_medida and ff.id_forma_farmaceutica=m.forma_farmaceutica and tm.id_med=m.serial and tm.id_tari='"+(String)session.getAttribute("tarifariomed")+"' "+where+" ORDER BY "+sidx + " " +sord +" LIMIT "+start+" , "+limit;
