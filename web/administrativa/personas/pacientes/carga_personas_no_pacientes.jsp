<%-- 
    Document   : carga_personas_no_pacientes
    Created on : 25/04/2013, 11:49:47 AM
    Author     : EMMANUEL
--%>
...
<%@page import="pac.cla" %>
  ...
  <%                    request.setCharacterEncoding("UTF-8");
                         ...
                         //  System.out.println(where);
                            ...
                            total = conexiondb.countRec("serial", "personas where serial not in (select serial_persona from pacientes) and  serial!='"+cl.idsuper()+"' "+where+"");
                          //     System.out.println("total "+total);
                            ...
                            strQuery = "select serial,id_person,p_nom,s_nom,p_ape,s_ape from personas  where  serial not in (select serial_persona from pacientes) and  serial!='"+cl.idsuper()+"'  "+where+" ORDER BY "+sidx + " " +sord +" LIMIT "+start+" , "+limit;
                            // System.out.println(strQuery);
                            
                            rs = conexiondb.Consulta(strQuery);
                            
                       //     total = conexiondb.countRec("serial", "personas  where  serial not in (select serial_persona from pacientes) "+where+"");
