...
<%
        ...
        try {
            ...
            for (int i = 0; i < Integer.parseInt("" + msg.size()); i++) {   ///ago un ciclo para todad las restricciones
                ...
                String campo = (String) jsonObject2.get("field");
                     if (campo.compareTo("idformula") == 0) { campo = "fm.id_consulta"; } 
                else if (campo.compareTo("id") == 0) { campo = "pe.id_person"; } 
                else if (campo.compareTo("serial") == 0) { campo = "pe.id_person"; } 	
                else if (campo.compareTo("id") == 0) { campo = "pe.serial"; } 		
                else if (campo.compareTo("cont") == 0) { campo = "ag.id_contrato"; } 	
                else if (campo.compareTo("paq") == 0) { campo = "ag.id_paquete"; } 
                else if (campo.compareTo("fecha") == 0) { campo = "cm.fecha"; } 
    ...
    total = conexiondb.countRec("Distinct (fm.id_consulta)", "formula_medica fm, personas pe,consulta_medica cm,historia_clinica hc, pacientes pa, agenda ag where cm.id_consulta=fm.id_consulta and hc.serial_peciente=cm.serial_historia and pa.serial_persona=hc.serial_peciente and pa.serial_persona=pe.serial and ag.id_consulta=cm.id_consulta " + where);
    ...
    strQuery = "select  Distinct (fm.id_consulta) as id_cons,pe.p_nom,pe.s_nom,p_ape,s_ape,pe.id_person,pe.serial,ag.id_contrato,ag.id_paquete,cm.fecha from formula_medica fm, personas pe,consulta_medica cm,historia_clinica hc, pacientes pa, agenda ag where cm.id_consulta=fm.id_consulta and hc.serial_peciente=cm.serial_historia and pa.serial_persona=hc.serial_peciente and pa.serial_persona=pe.serial and ag.id_consulta=cm.id_consulta " + where + " ORDER BY " + sidx + " " + sord + " LIMIT " + start + " , " + limit;
 //  out.println(strQuery + "\n");
    ...
    try {
        while (rs.next()) {
            ...
            json = json + "\n{";
            json = json + "\"id\":\"" + rs.getInt("id_cons") + "\",";
            json = json + "\"cell\":[" + rs.getInt("id_cons") + "";
            ...
            json = json + ",\"" + rs.getString("id_paquete") + "\"";
