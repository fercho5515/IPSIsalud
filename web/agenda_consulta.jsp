...
<script type="text/javascript" language="javascript">
  ...
  function cierra(){
   ...
   function atiende(dato,dato2,dato3,dato4,dato5,dato6){
                                ...
                                if(dato2=="Programada"){$( "#dialog1" ).dialog( "open" );}
                                   ...
                                   document.getElementById("id_contrato").value=dato6;
                                //   alert("idconsulta "+dato4);
                                   <%  int permi=0;
                                       
                                          if(request.getParameter( "permiso" )!=null){permi=Integer.parseInt(request.getParameter( "permiso" ));}
                                       else if(session.getAttribute("permiso")!=null){permi=Integer.parseInt((String)session.getAttribute("permiso"));}
                                  //     System.out.println("sera :"+permi);
                                       if(permi==1||permi==2||permi==3){
                                        out.print("document.forms.form1.action='consulta/contenedor.jsp';");
                                       }
                                       else if(permi==4){
                                         out.print("document.forms.form1.action='consulta_nutricion/consulta/contenedor_nutricion.jsp';");
                                       }
                                       else if(permi==14){
                                         out.print("document.forms.form1.action='psicologia/visita/contenedorpsicologia.jsp';");
                                             }
                                       else if(permi==15){
                                         out.print("document.forms.form1.action='trabajo_social/visita/contenedor_trabajo_social.jsp';");
                                             }
                                       else if(permi==6){
                                         out.print("document.forms.form1.action='trabajo_social/visita/contenedor_trabajo_social.jsp';");
                                             }
	...
	datatype: "json",
   	colNames:['Atender','Serial persona','Hora','Documento','Primer Nombre','Segundo Nombre','Primer Apellido','Segundo Apellido','Estado','consu','autoriza','am_pm','codigo contrato'],
   	colModel:[
		...
		{name:'p.s_ape',index:'p.s_ape', width:((anchoPantalla*16)/100),search:true, editable: true,required : true,editoptions: {maxlength: 200},editrules: { required: true },searchoptions:{sopt:['cn','eq']}},
   		...
   		{name:'codcon',index:'codcon', hidden:true,search:false,editable:false,required :false,editoptions:{maxlength: 63},editrules: { required: false }},
   	         ],
                        ...
                        for (var i = 0; i < ids.length; i++)
                        {
                            ...
                            var ret = jQuery("#list1").jqGrid('getRowData',ids[i]);
                              ...
                              if(ret.descripcion=="Atendida"){
                                           be = "<input style='height:22px;width:75px;color: red'  id='aceptar' type='button'  value='Atendida' onclick=\"atiende('" + cl + "','"+ret.descripcion+"','"+ret.serial+"','"+ret.idconsu+"','"+ret.num_autorizacion+"','"+ret.codcon+"');\"  />";
                           
                          }
                              else if(ret.descripcion=="Programada"){
                                           be = "<input style='height:22px;width:75px;blue: red'  id='aceptar' type='button' value='Atender' onclick=\"atiende('" + cl + "','"+ret.descripcion+"','"+ret.serial+"','"+ret.idconsu+"','"+ret.num_autorizacion+"','"+ret.codcon+"');\"  />";
                              }
                           else{ be = "<input style='height:22px;width:75px;color:green'  id='aceptar' type='button' value='Atender' onclick=\"atiende('" + cl + "','"+ret.descripcion+"','"+ret.serial+"','"+ret.idconsu+"','"+ret.num_autorizacion+"','"+ret.codcon+"');\"  />";} 
