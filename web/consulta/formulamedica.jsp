...
$(document).ready(function(){
    ...
    // select DISTINCT m.serial,m.producto,,m.cod_tres,v.descripcion,f.descripcion,m.concentracion,u.descripcion from medicamentos m,forma_farmaceutica f,via_admin v,unidad_medida u where m.forma_farmaceutica=f.id_forma_farmaceutica and m.via_administracion=v.id_via_admin and m.unidad_medida=u.id_unidad_medida
        colNames:['serial','cod atc','cod dos','cod tres','Descripción','Via Administración','Forma Farmacéutica','Concentración'],
   	colModel:[
                {name:'id',index:'id',hidden:true,width:0,editable: false,required : true,editoptions:{maxlength: 63}},
   		{name:'cod_atc',index:'cod_atc', width:60,search:true, editable: true,required : true,editoptions: {maxlength: 43},editrules: { required: true },searchoptions:{sopt:['cn','eq']}},
   		{name:'cod_dos',index:'cod_dos', width:60,search:true, editable: true,required : true,editoptions: {maxlength: 43},editrules: { required: true },searchoptions:{sopt:['cn','eq']}},
   		{name:'cod_tres',index:'cod_tres', width:60,search:true, editable: true,required : true,editoptions: {maxlength: 43},editrules: { required: true },searchoptions:{sopt:['cn','eq']}},
