<%-- 
    Document   : carga_resul_img
    Created on : 17/05/2013, 05:35:40 PM
    Author     : EMMANUEL
--%>
...
<%@page import="pac.conexion" %>
  ...
  <%
                         ...
                         //  System.out.println(where);
                            ...
                            total = conexiondb.countRec("ls.serial ", "imgs_sol ls,cups c,consulta_medica cm,personas per where id_cups=id_img and leido=0 and cm.id_consulta=ls.id_consulta and cm.id_estado_consulta='3' and cm.serial_historia='"+session.getAttribute("serial_pacinete")+"' and ls.serial not in (select id_sol_ima from resu_img) and per.serial=cm.id_medico "+where+"");
                            ...
                            strQuery = "select ls.serial,id_img,cup_verdadero,descripcion,descri,fecha_soli,concat(per.p_nom,' ',per.s_nom,' ',per.p_ape,' ',per.s_ape) as nmed,ls.archi_img from imgs_sol ls,cups c,consulta_medica cm,personas per where id_cups=id_img and leido=0 and cm.id_consulta=ls.id_consulta and cm.id_estado_consulta='3' and cm.serial_historia='"+session.getAttribute("serial_pacinete")+"' and ls.serial not in (select id_sol_ima from resu_img) and per.serial=cm.id_medico "+where+" ORDER BY "+sidx + " " +sord +" LIMIT "+start+" , "+limit;
   // System.out.println("carga resul img ==>"+strQuery);
                           ...
                           // total = conexiondb.countRec("ls.serial ", "imgs_sol ls,cups c,consulta_medica cm,personas per where id_cups=id_img and leido=0 and cm.id_consulta=ls.id_consulta and cm.id_estado_consulta='3' and cm.serial_historia='"+session.getAttribute("serial_pacinete")+"' and ls.serial not in (select id_sol_ima from resu_img) and per.serial=cm.id_medico "+where+"");
                            ...
                            while(rs.next()){
                                ...
                                json = json + ",\""+rs.getString("archi_img")+"\"]";
                                json = json + "}";
