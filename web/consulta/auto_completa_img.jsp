<%-- 
    Document   : auto_completa_img
    Created on : 17/05/2013, 11:57:04 AM
    Author     : EMMANUEL
--%>
...
String dato=request.getParameter( "searchword" );
           //select id_cups,cup_verdadero,descripcion from cups where id_cups not in (select DISTINCT id_lab from labs_sol where serial_historia='10001' and leido='0')  ORDER BY cup_verdadero desc LIMIT 0 , 15
                      String sql="select id_cups,cup_verdadero,descripcion from  cups where  (tipo='0' or tipo='1') and permi"+session.getAttribute("permiso")+"='1' and (sexo='"+session.getAttribute("genero_paciente")+"' or sexo='0') and id_cups not in (select DISTINCT id_img from imgs_sol where serial_historia='"+session.getAttribute("serial_pacinete")+"' and leido='0') and cup_verdadero like '"+dato+"%' LIMIT 15";
String sql2="select id_cups,count(cup_verdadero),descripcion,cup_verdadero from  cups where  (tipo='0' or tipo='1') and permi"+session.getAttribute("permiso")+"='1' and (sexo='"+session.getAttribute("genero_paciente")+"' or sexo='0') and id_cups not in (select DISTINCT id_img from imgs_sol where serial_historia='"+session.getAttribute("serial_pacinete")+"' and leido='0') and cup_verdadero like '"+dato+"%'";
...
//System.out.println(" sql1:"+sql+"\n sql2:"+sql2);
 ...
 try{
	    while (resu2.next()&&resu2!=null) {
                           out.print(" <input type='hidden' name='id_r_img' id='id_r_img' value='"+resu2.getString(1)+"'>");
                           ...
                           out.print(" <input type='hidden' name='codigoimg' id='codigoimg' value='"+resu2.getString(4)+"'>");              
                  }
             }
 ...
 ResultSet resu=ba.consultas(sql);
	...
	try{
	    while (resu.next()&&resu!=null) { 
                        %>
                        <div class="display_box" align="left" onclick="pasadatoimg('<% out.print(resu.getString(1));%>','<% out.print(resu.getString(2));%>','<% out.print(resu.getString(3));%>')">
