    ...
    <center> 
        ...
        <%
         ...
         if(session.getAttribute("nombre")==null||session.getAttribute("clave")==null||session.getAttribute("id")==null || session.getAttribute("permiso")==null || session.getAttribute("serial_agenda" )==null || session.getAttribute("serial_pacinete")==null || session.getAttribute("serial_consulta")==null ){}
                ...
                ba.transaccion("DELETE FROM `ips_isalud`.`histo_subsig` WHERE `id_consulta`='"+session.getAttribute("serial_consulta")+"';");
                ba.transaccion("DELETE FROM `ips_isalud`.`impre_diagno_trabajo_soci` WHERE `id_consulta_trabajo_soci`='"+session.getAttribute("serial_consulta")+"';");
                ba.transaccion("DELETE FROM `ips_isalud`.`comp_fami` WHERE `id_cons_trab_soci`='"+session.getAttribute("serial_consulta")+"';");
                ba.transaccion("DELETE FROM `ips_isalud`.`archivos_consulta_trabajo_so` WHERE `id_consulta`='"+session.getAttribute("serial_consulta")+"';");
